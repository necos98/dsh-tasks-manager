...
// close is user-only and frees the slot. The queued order is DERIVED from
// queued_at (never stored as an index): moveQueued rewrites the stamps of the
// whole queued set and never promotes. No git/test/PR logic here.
...
function promoteRow(db, workspaceId, row) {
  ...
  let n = 2;
  while (clash.get(workspaceId, branch, "draft")) { branch = branchFor(row.seq, row.slug) + "-" + n; n += 1; }
