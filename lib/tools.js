...
import { TaskError, approve, claimUnboundActive, close, enqueue, ensureWorkspace, list, resolveTask, search } from "./queue.js";
...
function mutationRender(verb) {
  return (_args, value) => [{
    ...
    text: verb + " " + fmt(value.task) + (value.promoted && value.promoted.id !== value.task.id ? " | promoted #" + num(value.promoted) + " -> active (" + value.promoted.branch + ")" : ""),
  }];
}
  ...
  return [
    ...
    defineTool({
      ...
      execute: async (args, exec) => {
        const c = ctxOf(store, exec);
        const owned = ownedTask(c.db, c.workspaceRowId, args.id);
        return close(c.db, owned.id, args.outcome);
      },
    }),
  ];
}
