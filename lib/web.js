...
import { workerCanFinishOf, workerCanMergeOf, TASKS_NS } from "./finish-toggle.js";
...
function snapshotOf(c, modes) {
  ...
  return { workspace: c.workspace, tasks: rows, ...modes };
}
...
export function createWebHandlers(store, hooks) {
  ...
  // Finish/merge modes for the panel header (both manual default): read
  // live from the plugin ctx settings; unit-test handlers without ctx
  // read as manual.
  function modes() {
    try {
      ...
      return { workerCanFinish: workerCanFinishOf(value), workerCanMerge: workerCanMergeOf(value) };
    } catch {
      return { workerCanFinish: false, workerCanMerge: false };
    }
  }
  return {
    snapshot(payload) {
