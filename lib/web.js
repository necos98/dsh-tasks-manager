...
// (ctx.connection.rpc.handle, same pattern as dsh-plugin-manager; lib/index.js
// carries the one-line context workaround that makes register() see webServer).
...
//   updateStatus / checkUpdate / applyUpdate — the manual GitHub updater
//                                       (lib/updater.js). Deployment-wide, not
//                                       per workspace: no sessionId, no queue
//                                       or DB access. They come from
//                                       hooks.updater; without it every call
//                                       answers "the updater is not mounted".
...
/** No updater mounted: a state the page renders, not an exception. */
async function updaterNotMounted() {
  return { ok: false, error: { code: "internal", message: "the updater is not mounted", details: {} } };
}
...
export function createWebHandlers(store, hooks) {
  ...
  const updater = hooks && typeof hooks.updater === "function" ? hooks.updater : updaterNotMounted;
  ...
  // Manual GitHub updater: the endpoint answers are already RPC answers
  // ({ ok, value } | { ok, error }), so they are returned unwrapped. A throw
  // inside the updater still reads as an answer, never as a broken channel.
  async function updaterCall(endpoint, payload) {
    try {
      return await updater(endpoint, payload);
    } catch (error) {
  ...
  return {
    ...
    // Deployment-wide updater endpoints (lib/updater.js): no sessionId, no
    // workspace ownership guard, no DB access.
    updateStatus(payload) {
      return updaterCall("updateStatus", payload);
    },
    checkUpdate(payload) {
      return updaterCall("checkUpdate", payload);
    },
    applyUpdate(payload) {
      return updaterCall("applyUpdate", payload);
    },
  };
}
