...
import { TaskError, approve, close, ensureWorkspace, get, list } from "./queue.js";
...
// BEFORE any mutation runs. Cross-workspace ids read as not-found.
function ownedTask(db, workspaceRowId, id) {
  const t = get(db, id);
  if (!t || t.workspace_id !== workspaceRowId) {
    throw new TaskError("not-found", "no task #" + id + " here");
  }
  return t;
}
...
function snapshotOf(c) {
  ...
  return { workspace: c.workspace, tasks: rows };
}
...
export function createWebHandlers(store, hooks) {
  ...
  // Fire-and-forget worker spawn after a promotion to active. The queue
  // mutation already committed; a spawn failure must never roll it back —
  // the task stays active and the error rides the payload as spawnError.
  function maybeSpawn(c, out) {
    ...
    return spawner({ ctx: pluginCtx, db: c.db, workspace: c.workspace, task: active })
  ...
  return {
    snapshot(payload) {
      ...
      return snapshotOf(workspaceCtx(store, sid));
    },
    ...
    async close(payload) {
      ...
      const c = workspaceCtx(store, sid);
      ownedTask(c.db, c.workspaceRowId, id);
      const out = await close(c.db, id, outcome);
      const task = get(c.db, id);
      const promoted = out.promoted ? get(c.db, out.promoted.id) : null;
