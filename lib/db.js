...
function applySchema(db) {
  ...
  if (version !== 0 && version !== SCHEMA_VERSION) {
    if (version === 1) { migrateV1toV2(db); migrateV2toV3(db); db.exec("PRAGMA user_version = 3"); }
    else if (version === 2) { migrateV2toV3(db); db.exec("PRAGMA user_version = 3"); }
    else throw new Error("tasks: unsupported schema " + version);
    return;
  }
  if (version === 0) { migrateV3(db); db.exec("PRAGMA user_version = 3"); }
}
