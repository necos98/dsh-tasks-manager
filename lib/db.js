...
function applySchema(db) {
  ...
  if (version !== 0 && version !== SCHEMA_VERSION) {
    if (version === 1) { migrateV1toV2(db); migrateV2toV3(db); migrateV3toV4(db); migrateV4toV5(db); migrateV5toV6(db); db.exec("PRAGMA user_version = 6"); }
    else if (version === 2) { migrateV2toV3(db); migrateV3toV4(db); migrateV4toV5(db); migrateV5toV6(db); db.exec("PRAGMA user_version = 6"); }
    else if (version === 3) { migrateV3toV4(db); migrateV4toV5(db); migrateV5toV6(db); db.exec("PRAGMA user_version = 6"); }
    else if (version === 4) { migrateV4toV5(db); migrateV5toV6(db); db.exec("PRAGMA user_version = 6"); }
    else if (version === 5) { migrateV5toV6(db); db.exec("PRAGMA user_version = 6"); }
    else throw new Error("tasks: unsupported schema " + version);
    return;
  }
  if (version === 0) { migrateV3(db); db.exec("PRAGMA user_version = 6"); }
}
...
function migrateV3(db) {
  db.exec("CREATE TABLE workspaces (id INTEGER PRIMARY KEY, path TEXT NOT NULL UNIQUE, base_branch TEXT NOT NULL DEFAULT '', queue_enabled INTEGER NOT NULL DEFAULT 1, created_at TEXT NOT NULL, updated_at TEXT NOT NULL)");
...
// v5 adds the per-project queue switch: workspaces.queue_enabled (1 = the
// queue advances on its own, the default and the pre-v5 behavior; 0 = paused,
// approve only queues and the user starts one task by hand). Fresh installs
// create the column directly in migrateV3; older databases get it here, and
// every existing workspace reads 1 so nothing changes for them.
function migrateV4toV5(db) {
  db.exec("ALTER TABLE workspaces ADD COLUMN queue_enabled INTEGER NOT NULL DEFAULT 1");
}

// v6 adds the per-task notes log: tasks.notes, an append-only list of
// "- [ISO timestamp] body" entries written by the worker bound to the task
// (lib/queue.js appendNote). NOT NULL DEFAULT '' (never NULL) on purpose:
// every existing row reads as "no notes", so readers need no null handling
// and the panel/tool projections can treat the column as a plain string.
// Fresh installs create the column directly in migrateV3.
