...
function applySchema(db) {
  ...
  if (version !== 0 && version !== SCHEMA_VERSION) {
    if (version === 1) { migrateV1toV2(db); migrateV2toV3(db); migrateV3toV4(db); migrateV4toV5(db); db.exec("PRAGMA user_version = 5"); }
    else if (version === 2) { migrateV2toV3(db); migrateV3toV4(db); migrateV4toV5(db); db.exec("PRAGMA user_version = 5"); }
    else if (version === 3) { migrateV3toV4(db); migrateV4toV5(db); db.exec("PRAGMA user_version = 5"); }
    else if (version === 4) { migrateV4toV5(db); db.exec("PRAGMA user_version = 5"); }
    else throw new Error("tasks: unsupported schema " + version);
    return;
  }
  if (version === 0) { migrateV3(db); db.exec("PRAGMA user_version = 5"); }
}
