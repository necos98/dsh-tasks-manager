...
// with Expand + timestamps, plus a read-only "Notes" block (the worker's
// append-only notes log, rendered inside the card's existing expand/collapse)
// when the task has any. Drafts offer Approve (direct) + Discard (confirm);
// queued/active offer Close with done|cancelled|failed choice (confirm), and
// an active card also offers Back to queue (requeue: frees the slot, keeps the
// branch, clears the worker session).
...
// approve/close/start/setQueueEnabled/move/requeue stay USER-ONLY: they travel the
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const cssText =
      ...
      "._tskNotes{display:flex;flex-direction:column;gap:4px;border-left:2px solid var(--dsw-alias-border-l2);padding-left:8px}" +
      "._tskNotesTitle{margin:0;font-size:11px;font-weight:600;letter-spacing:.04em;text-transform:uppercase;color:var(--dsw-alias-label-tertiary)}" +
      ...
      "._tskSetCard{display:flex;flex-direction:column;gap:16px}" +
      ...
      "._tskSetGroup{display:flex;flex-direction:column;gap:10px;border:1px solid var(--dsw-alias-border-l2);border-radius:10px;padding:12px;background:var(--dsw-alias-bg-layer-1)}" +
      "._tskSetGroupTitle{margin:0;font-size:12px;font-weight:600;color:var(--dsw-alias-label-secondary);text-transform:uppercase;letter-spacing:.04em}" +
      "._tskSetRow{display:flex;align-items:flex-start;gap:12px}" +
      "._tskSetLabelCol{flex:1;display:flex;flex-direction:column;gap:2px;min-width:0}" +
      "._tskSetLabel{font-size:13px;font-weight:600;color:var(--dsw-alias-label-primary);display:inline-flex;align-items:center;gap:6px}" +
      "._tskSetCustomBadge{flex:none;background:var(--dsw-alias-brand-primary);color:var(--dsw-alias-bg-base);border-radius:999px;padding:0 6px;font-size:10px;font-weight:600;line-height:16px;white-space:nowrap}" +
      "._tskSetHint{font-size:11px;line-height:16px;color:var(--dsw-alias-label-tertiary);margin:0}" +
      "._tskSetControlCol{flex:none;display:flex;flex-direction:column;align-items:flex-end;gap:4px}" +
      "._tskSetHelpBox{margin:0;font-size:11px;line-height:16px;color:var(--dsw-alias-label-secondary);background:var(--dsw-alias-bg-layer-1);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;padding:6px 10px;max-width:220px;white-space:pre-wrap}" +
      ...
      "._tskSelect{background:var(--dsw-alias-bg-layer-1);color:var(--dsw-alias-label-primary);border:1px solid var(--dsw-alias-border-l2);border-radius:8px;height:30px;font-size:12px;font:inherit;padding:0 8px;min-width:0;width:220px}" +
      "._tskGroupedSelect{width:240px;max-height:200px;overflow:auto}" +
      "._tskResetBtn{appearance:none;border:0;background:0 0;color:var(--dsw-alias-brand-primary);font-size:11px;cursor:pointer;padding:0;font:inherit;align-self:flex-end}" +
      "._tskResetBtn:hover{color:var(--dsw-alias-label-primary)}" +
      "._tskDefaultReset{align-self:flex-end;margin-top:auto;padding-top:4px}" +
    ...
    // Active cards get "Back to queue" (onRequeue): the task leaves the slot
    // without being closed, keeps its branch and re-enters at the end of the
    // FIFO (the promotion funnel may start the next queued task right away).
    function TaskCard({ task, busy, spawn, onApprove, onDiscard, onClose, onStart, onMove, onUnqueue, onRequeue, canMoveUp, canMoveDown, queueEnabled, t }) {
      ...
      // Worker notes log (schema v6): read-only, append-only, rendered in full
      // (no truncation) inside the card's own expand/collapse.
      ...
      const body = el(
        ...
        notes
          ? el(
              "div",
              { className: "_tskNotes" },
              el("div", { className: "_tskNotesTitle" }, t("notes")),
              el("p", { className: "_tskSpec" }, notes)
            )
          : null,
        el(
          "div",
        ...
        (task.state === "queued" || task.state === "active") && !closing
          ? el(
              ...
              task.state === "queued" && typeof onUnqueue === "function"
                ...
                : null,
              task.state === "active" && typeof onRequeue === "function"
                ? el(
                    "button",
                    {
                      type: "button",
    ...
    function TasksPanel({ sessionId, connection, runningOf, onClosePanel, t }) {
      ...
      const mutate = React.useCallback(
        async (endpoint, key, payload) => {
          ...
          try {
            ...
            // requeue folds in here too: freeing the slot promotes like close.
            if (value && value.spawn && (endpoint === "approve" || endpoint === "close" || endpoint === "start" || endpoint === "requeue")) {
              const ids = [];
      ...
      // Every request that carries a task number sends the VISIBLE #N (the
      // per-workspace seq the card shows), never the internal row id: in a DB
      // where ids and seqs have diverged the server would otherwise resolve a
      // number that names a different card. Local-only keys (React key, busy,
      // spawnInfo) stay on the internal task.id: they never leave the client
      // and are matched against the server-returned value.task.id.
      const visibleNum = (task) => task.seq ?? task.id;

      const onApprove = (task) =>
        mutate("approve", "approve:" + task.id, { sessionId, id: visibleNum(task) });
      const onDiscard = (task) => setConfirmDiscard(visibleNum(task));
      const confirmDiscardNow = (task) => {
        setConfirmDiscard(null);
        mutate("close", "close:" + task.id, { sessionId, id: visibleNum(task), outcome: "cancelled" });
      };
      const onClose = (task, outcome) =>
        mutate("close", "close:" + task.id, { sessionId, id: visibleNum(task), outcome });
      // Manual start: promotes exactly this queued task (the paused path).
      const onStart = (task) =>
        mutate("start", "start:" + task.id, { sessionId, id: visibleNum(task) });
      ...
      const onMove = (task, direction) =>
        mutate("move", "move:" + task.id, { sessionId, id: visibleNum(task), direction });
      ...
      const onUnqueue = (task) =>
        mutate("unqueue", "unqueue:" + task.id, { sessionId, id: visibleNum(task) });
      // Back to queue: frees the slot of an active task without closing it
      // (the worker chat stays open but detached) so the FIFO head can start.
      // mutate() refresh moves the card into the Queued group, at the end.
      const onRequeue = (task) =>
        mutate("requeue", "requeue:" + task.id, { sessionId, id: visibleNum(task) });
                    ...
                    "button",
                    {
                      ...
                      onClick: () => {
                        const task = (snapshot ? snapshot.tasks : []).find(
                          (row) => (row.seq ?? row.id) === confirmDiscard
                        );
    ...
    function apply(ctx) {
          ...
          () =>
            locale.register(NS, {
              en: {
                ...
                setTitle: "Tasks",
                setDesc: "One active task per workspace, FIFO promotion. Grouped into two sections below: how a task ends (Flow) and how the worker runs (Worker).",
                setGroupFlow: "Flow",
                setFlowHint: "Whether the worker closes its own task or you close it from the panel. Whether the worker merges into the base branch or you do it outside.",
                setFinishLabel: "Worker closes its own task",
                setFinishOn: "ON → worker closes its task on finish, queue advances automatically.",
                setFinishOff: "OFF (default) → you close tasks from the panel, worker only reports ready.",
                setMergeLabel: "Worker merges into base",
                setMergeOn: "ON → after rebase + green tests, does a clean `git merge --no-ff` into base; on any conflict it aborts and leaves the task open for you.",
                setMergeOff: "OFF (default) → the worker never touches base; you merge outside when ready.",
                setGroupWorker: "Worker",
                setWorkerHint: "Per-task overrides applied when the worker chat is spawned. Leave fields empty to inherit the global defaults.",
                setBranchLabel: "Base branch",
                setBranchHint: "Leave empty for auto (origin/HEAD).",
                setBranchPlaceholder: "e.g. main",
                setModelLabel: "Worker model",
                setModelHint: "Pick a model for the worker from its provider's catalog, or keep the global default. Empty = inherit the global model selection.",
                setModelPlaceholder: "default",
                setModelDefault: "default (global)",
                setRulesLabel: "Extra instructions",
                setRulesHint: "Free text appended to the worker's first message, not to the system prompt. One instruction per line.",
                setRulesPlaceholder: "e.g. at the end of the work update the wiki",
                setCustom: "custom",
                setResetAll: "Restore defaults",
                ...
                setUpdatesTitle: "Updates",
                setUpdatesHint: "Manual check against the GitHub release tags of this plugin. Nothing is read, and no install runs, until you click Check for updates.",
                updaterCurrent: "installed",
                updaterLatest: "newest",
                updaterCheck: "Check for updates",
                updaterChecking: "Reading the GitHub tags…",
                updaterUpdate: "Update to {tag}",
                updaterUpdating: "Installing…",
                updaterUpToDate: "Already the newest release.",
                updaterNotInstalled: "Not installed in this profile: the update installs it.",
                updaterNoRelease: "No release yet.",
                updaterError: "The tag lookup failed.",
                updaterRestartTitle: "Restart required",
                updaterRestartHint: "The new version is on disk; restart `dsh web` to load it.",
                updaterUpdatedTo: "Updated to {version}",
                updaterAlreadyNewest: "The newest release was already on disk.",
                updaterLinked: "installed from a local link: update it in its own checkout",
                updaterUnavailable: "This host half does not serve the updater yet: restart `dsh web` and reload the page.",
              },
            }),
      ...
      // Dedicated settings section for the Tasks plugin: one top-level
      // `settings.section` page ("Tasks"), not a card inside the built-in
      // Plugins section. Direct writes through the bound settings scope.
      if (slots !== undefined && settingsScope !== undefined) {
        ...
        console.warn(
          "[dsh-tasks-manager] settingsScope unavailable - the tasks settings section will not render"
        );
      }
    }
    // #endregion
    ...
    // Manual GitHub updater block (Settings → Tasks). It reads the release
    // tags of the plugin repository ONLY when the user clicks Check for
    // updates: mounting issues one updateStatus call (no network, no shell).
    // The three endpoints are deployment-wide, so this block never touches
    // the settings scope and is never disabled by `snap.writable`.
    // A failing updateStatus means the running host half does not serve the
    // updater (older host next to a freshly installed client bundle): that is
    // rendered as the `updaterUnavailable` state, never as an exception.
    function TasksUpdaterBlock(props) {
      ...
      const [error, setError] = React.useState(null);
      const failure = (cause) => (cause instanceof Error ? cause.message : String(cause));
      ...
      React.useEffect(() => {
        ...
        (async () => {
          try {
            ...
            const value = await call(connection, "updateStatus", {});
      ...
      const check = async () => {
        ...
        setChecking(true);
        try {
          setResult(await call(connection, "checkUpdate", {}));
        } catch (cause) {
      ...
      const apply = async () => {
        ...
        setApplying(true);
        try {
          setInstalled(await call(connection, "applyUpdate", {}));
          setResult(null);
        } catch (cause) {
          setError(failure(cause));
        } finally {
      ...
      if (unavailable) {
        return el(
          "div",
          { className: "_tskSetGroup" },
          el("h4", { className: "_tskSetGroupTitle" }, t("setUpdatesTitle")),
          el("p", { className: "_tskSetHint" }, t("updaterUnavailable"))
        );
      }

      const current = installed && typeof installed.current === "string"
        ? installed.current
        : (status && typeof status.current === "string" ? status.current : "—");
      const latest = (result && typeof result.latest === "string" ? result.latest : "")
        || (installed && typeof installed.latest === "string" ? installed.latest : "");
      const canInstall = installed === null && result !== null
        && (result.status === "update-available" || result.status === "not-installed")
        && typeof result.latest === "string";

      const badges = [el("span", { className: "_tskBadge", key: "current" }, t("updaterCurrent") + " " + current)];
      if (latest !== "") badges.push(el("span", { className: "_tskBadge", key: "latest" }, t("updaterLatest") + " " + latest));
      if (status && status.linked === true) badges.push(el("span", { className: "_tskBadge", key: "linked" }, t("updaterLinked")));

      const lines = [];
      if (installed !== null) {
        lines.push(el(
          "p",
          { className: "_tskMeta", key: "installed" },
          installed.updated === true
            ? t("updaterUpdatedTo", { version: installed.current ?? installed.latest })
            : t("updaterAlreadyNewest")
        ));
      } else if (result !== null) {
        const summary = {
          "up-to-date": "updaterUpToDate",
          "not-installed": "updaterNotInstalled",
          "no-release": "updaterNoRelease",
          error: "updaterError",
        }[result.status];
        if (summary) lines.push(el("p", { className: "_tskMeta", key: "summary" }, t(summary)));
        const notes = Array.isArray(result.notes) ? result.notes : [];
        for (let i = 0; i < notes.length; i += 1) {
          lines.push(el("p", { className: "_tskMeta", key: "note-" + i }, notes[i]));
        }
      }
      if (installed !== null && installed.restartRequired === true) {
        lines.push(el("div", { key: "restart" },
          el("p", { className: "_tskSetLabel" }, t("updaterRestartTitle")),
          el("p", { className: "_tskSetHint" }, t("updaterRestartHint"))
        ));
      }
      ...
      return el(
        "div",
        { className: "_tskSetGroup" },
        el("h4", { className: "_tskSetGroupTitle" }, t("setUpdatesTitle") + " — " + t("setUpdatesHint")),
        el(
          "div",
          { className: "_tskSetRow" },
          el("div", { className: "_tskSetLabelCol" }, el("div", { className: "_tskBadges" }, badges)),
          el(
            "div",
            { className: "_tskSetControlCol" },
            el(
              "div",
              { className: "_tskActions" },
              el(
                ...
                checking ? t("updaterChecking") : t("updaterCheck")
              ),
              canInstall ? el(
                "button",
                {
                ...
                applying ? t("updaterUpdating") : t("updaterUpdate", { tag: result.latest })
              ) : null
            )
          )
        ),
        lines.length > 0 ? el("div", { className: "_tskSetLabelCol" }, lines) : null,
        error !== null
          ? el("div", { className: "_tskError" }, el("p", { className: "_tskErrorText", role: "alert" }, error))
          : null
      );
    }

    function TasksSettingsCard(props) {
      const { t, settingsScope, connection } = props;
      ...
      const [modelRoutes, setModelRoutes] = React.useState([]);
      const [modelRoutesLoading, setModelRoutesLoading] = React.useState(true);
      ...
      const effectiveModel = typeof value.workerModel === "string" ? value.workerModel : "";
      const modelOverridden = Object.hasOwn(user, "workerModel");
      const modelText = modelDraft !== null ? modelDraft : effectiveModel;
      ...
      // Worker-model picker: pull the registered DSH LLM routes once and turn
      // the text input into a native select (falling back to the input when the
      // list is empty). The select value is "provider/model"; "" = global default.
      React.useEffect(() => {
        let cancelled = false;
        (async () => {
          try {
            if (!connection) throw new Error("no connection");
            const value = await call(connection, "modelRoutes", {});
            if (cancelled) return;
            const providers = Array.isArray(value && value.providers) ? value.providers : [];
            const routes = [];
              ...
              if (models.length > 0) {
                for (const model of models) {
                  ...
                  routes.push({
                    provider: pid,
                    model: mid,
                    name: typeof model.name === "string" && model.name !== "" ? model.name : mid,
                    description: typeof model.description === "string" ? model.description : "",
                  });
                }
              } else {
                // Provider with no advertised models: offer the provider route itself.
                routes.push({
                  provider: pid,
                  model: "",
                  name: (provider.name !== undefined && provider.name !== "") ? provider.name : pid,
                  description: "",
                });
              }
            }
            ...
            if (!cancelled) setModelRoutes([]);
          } finally {
            if (!cancelled) setModelRoutesLoading(false);
          }
        })();
        return () => { cancelled = true; };
      }, [connection]);
      ...
      const commitModel = () => {
        if (modelDraft === null || !writable) return;
        const trimmed = modelDraft.trim();
        setModelDraft(null);
        write(() => (trimmed === "" ? scope.unset("workerModel") : scope.set("workerModel", trimmed)));
      };
      const resetModel = () => { if (writable) { setModelDraft(null); write(() => scope.unset("workerModel")); } };
      const commitRules = () => {
      ...
      const resetAll = () => {
        ...
        setRulesDraft(null);
        write(() => {
          scope.unset("workerCanFinish");
          scope.unset("workerCanMerge");
          scope.unset("baseBranch");
          scope.unset("workerModel");
          scope.unset("workerRules");
        });
      };
      const anyOverridden =
        finishOverridden || mergeOverridden || branchOverridden || modelOverridden || rulesOverridden;

      const switchRow = (label, on, overridden, help, toggle, trailing) =>
        el(
          "div",
          { className: "_tskSetRow", key: label },
          el(
            "div",
            { className: "_tskSetLabelCol" },
            el(
              "span",
              { className: "_tskSetLabel" },
              label,
              overridden ? el("span", { className: "_tskSetCustomBadge" }, t("setCustom")) : null
            ),
            el("p", { className: "_tskSetHelpBox" }, help)
          ),
          el(
            "div",
            { className: "_tskSetControlCol" },
            el(
              "button",
              {
                ...
                "aria-label": label,
                disabled: !writable,
                className: "_tskSwitch" + (on ? " _tskSwitchOn" : ""),
                onClick: toggle,
              },
              el("span", { className: "_tskKnob" })
            ),
            overridden ? trailing : null
          )
        );

      const textRow = (label, hint, overridden, input, resetKey) =>
        el(
          "div",
          { className: "_tskSetRow", key: label },
          el(
            ...
            el("p", { className: "_tskSetHint" }, hint)
          ),
          el(
            "div",
            { className: "_tskSetControlCol" },
            ...
            overridden ? el(
              "button",
              { type: "button", className: "_tskResetBtn", onClick: resetKey, disabled: !writable },
              t("setResetAll")
            ) : null
          )
        );

      // Worker-model picker: a native <select> filled from the registered DSH
      // LLM routes. Each option's value is the model id exactly as the owning
      // provider advertises it (no "provider/" prefix): the `workerModel`
      // setting is fed to agents.create as the `model` field, and the pi-ai
      // provider resolves it inside its OWN catalog, where the id is the bare
      // model name. When a provider advertises no models the option value is
      // the provider route itself. "" always means the global default.
      const modelOptions = [];
      if (modelRoutes.length > 0) {
        const grouped = new Map();
        for (const route of modelRoutes) {
          const group = grouped.get(route.provider) || {
            label: route.provider,
            models: [],
          };
          group.models.push(route);
          grouped.set(route.provider, group);
        }
        for (const [provider, group] of grouped) {
          const children = group.models.map((route, i) => {
            const hasModel = route.model !== "";
            // value = the bare model id (the provider's own catalog key), NOT
            // "provider/model": the worker spawn passes it as `model` and the
            // owning provider resolves it. Providers without a model catalog
            // fall back to the provider route id itself.
            const optionValue = hasModel ? route.model : route.provider;
            const optionLabel = hasModel ? route.name : (route.name + " (any)");
            return el("option", {
              key: optionValue + "-" + i,
              value: optionValue,
            }, optionLabel);
          ...
          modelOptions.push(
            el("optgroup", { key: "pg-" + provider, label: group.label }, children)
          );
        }
      }
      const modelSelectEl = modelRoutes.length > 0
        ? el("select", {
            className: "_tskSelect _tskGroupedSelect",
            value: modelText,
            disabled: !writable,
            "aria-label": t("setModelLabel"),
            onChange: (e) => setModelDraft(e.target.value),
            onBlur: commitModel,
          },
            el("option", { value: "" }, t("setModelDefault")),
            modelOptions
          )
        : el("input", {
            className: "_tskInput",
            ...
            onKeyDown: (e) => {
              ...
              if (e.key === "Escape") setModelDraft(null);
            },
          });
      ...
      return el(
        ...
        !ready
          ? el("p", { className: "_tskMeta" }, t("loading"))
          : el(
              React.Fragment,
              null,
              el(
                "div",
                { className: "_tskSetGroup", key: "flow" },
                el(
                  "h4",
                  { className: "_tskSetGroupTitle" },
                  t("setGroupFlow") + " — " + t("setFlowHint")
                ),
                switchRow(
                  t("setFinishLabel"),
                  effectiveFinish,
                  finishOverridden,
                  effectiveFinish ? t("setFinishOn") : t("setFinishOff"),
                  toggleFinish,
                  finishOverridden ? el(
                    "button",
                    { type: "button", className: "_tskResetBtn", onClick: resetFinish, disabled: !writable },
                    t("setResetAll")
                  ) : null
                ),
                switchRow(
                  ...
                  effectiveMerge,
                  mergeOverridden,
                  effectiveMerge ? t("setMergeOn") : t("setMergeOff"),
                  toggleMerge,
                  mergeOverridden ? el(
                    "button",
                    { type: "button", className: "_tskResetBtn", onClick: resetMerge, disabled: !writable },
                    t("setResetAll")
                  ) : null
                )
              ),
              el(
                "div",
                { className: "_tskSetGroup", key: "worker" },
                el(
                  "h4",
                  { className: "_tskSetGroupTitle" },
                  t("setGroupWorker") + " — " + t("setWorkerHint")
                ),
                textRow(
                  t("setBranchLabel"),
                  t("setBranchHint"),
                ...
                textRow(
                  ...
                  el("textarea", {
                    ...
                    onBlur: commitRules,
                    onKeyDown: (e) => {
                      if (e.key === "Escape") setRulesDraft(null);
                    },
                  }),
                  resetRules
                )
              ...
              anyOverridden
                ? el(
                    "div",
                    { className: "_tskDefaultReset" },
                    el(
                      "button",
                      { type: "button", className: "_tskGhost", onClick: resetAll, disabled: !writable },
                      t("setResetAll")
                    )
                  )
                : null
            ),
        // Deployment-wide, settings-independent: always rendered, never
        // gated by `snap.writable` (it writes no setting).
        el(TasksUpdaterBlock, { key: "updates", t, connection })
      );
    }
    // #endregion
