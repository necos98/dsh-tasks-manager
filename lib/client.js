...
// shared /api RPC channel (ctx.connection.rpc.call with a "tasks-queue/" endpoint
// prefix, intercepted by the host half), never model tools.
//
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CHANNEL = "/api";
    /** Endpoint prefix the host half intercepts on CHANNEL. */
    const ENDPOINT_PREFIX = "tasks-queue/";
    ...
    async function call(connection, endpoint, payload) {
      const result = await connection.rpc.call(CHANNEL, ENDPOINT_PREFIX + endpoint, payload ?? {});
      if (!result.ok) {
