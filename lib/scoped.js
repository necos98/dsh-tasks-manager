...
//   intake: dsh-tasks-manager/intake-tools -> enqueue/edit/list/detail/search
//   worker: dsh-tasks-manager/worker-tools  -> get_my_task/finish_task/note_task/list/detail
// approve/close are never model-visible: no preset mounts them.
// finish_task is worker-only: close OWN active task (done|failed).
// note_task is worker-only and NOT gated by the finish switch: it changes no
// queue state, so it stays visible in manual mode too.
...
export const INTAKE_TOOLS = ["enqueue_task", "edit_draft", "list_tasks", "task_detail", "search_tasks"];
export const WORKER_TOOLS = ["get_my_task", "finish_task", "note_task", "list_tasks", "task_detail"];
